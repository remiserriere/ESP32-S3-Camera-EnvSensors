...
Three separate workflows run only when relevant files change — no wasted CI minutes.

### Firmware CI (`.github/workflows/ci.yml`)

Triggers **only** on push / pull-request touching ESP32 source files:
`src/**`, `include/**`, `lib/**`, `platformio.ini`, `scripts/**`
...
- Changes to `gauge-service/**` or documentation do **not** trigger this workflow.

### Gauge-service CI (`.github/workflows/gauge-service.yml`)

Triggers **only** on push / pull-request touching `gauge-service/**`.

- **`test` job** — installs Python 3.11 dependencies and runs the full pytest suite
- **`docker` job** (main branch push only, after tests pass) — builds the Docker image and pushes it to [GitHub Container Registry (GHCR)](https://ghcr.io) as `ghcr.io/<owner>/gauge-service:latest`
- Changes to ESP32 source do **not** trigger this workflow.
...
The **Release** workflow (`.github/workflows/release.yml`) runs on any `v*` tag and:

1. Builds the firmware with `FIRMWARE_VERSION = v1.2.3` baked in
2. Generates `manifest.json` pointing at the binary download URL
3. Creates a GitHub Release with `firmware-v1.2.3.bin`, `firmware-v1.2.3.elf`, `manifest.json`
4. Builds the gauge-service Docker image and pushes it to GHCR with both tags:
   - `ghcr.io/<owner>/gauge-service:v1.2.3` — immutable versioned image
   - `ghcr.io/<owner>/gauge-service:latest` — updated rolling tag

Tags containing `-alpha`, `-beta`, or `-rc` are automatically marked as pre-releases.

### Pulling the gauge-service Docker image
